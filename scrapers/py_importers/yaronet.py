...
def build(prod: Production, entrypath: str, desired_extentions: list):
    ...
    if not os.path.exists(entrypath + prod.slug):
        ...
        if prod.screenshots[0].endswith(".webp") or prod.screenshots[0].endswith(".WEBP"):
            screen_file_path += "webp"
        
        if prod.screenshots[0].endswith(".gif") or prod.screenshots[0].endswith(".GIF"):
            screen_file_path += "gif"

        if prod.screenshots[0].endswith(".png") or prod.screenshots[0].endswith(".PNG"):
            screen_file_path += "png"

        open(screen_file_path, 'wb').write(r.content)
        
        if prod.screenshots[0].endswith(".webp") or prod.screenshots[0].endswith(".gif") or prod.screenshots[0].endswith(".WEBP") or prod.screenshots[0].endswith(".GIF"):
            im = Image.open(screen_file_path).convert("RGB")
            im.save(filepath + prod.slug + ".png", "png")
            #os.remove(filepath + prod.slug + "." + "webp")
...
def autom_sources():
    fd = open(unzipped_source + "gb-thread-entries/gb-autom-source")
    lines = fd.readlines()
...
def manual_sources():
    fd = open(unzipped_source + "gb-thread-entries/gb-manual-source")
    lines = fd.readlines()
    lines = [line.strip() for line in lines]
...
def main():
    ...
    logger.write("[INFO]", " Scraping semi-automatic sources...")
    autom_sources()

    logger.write("[INFO]", " Scraping manual built sources...")
    manual_sources()
