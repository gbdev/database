...
try:
    with zipfile.ZipFile(unzipped_source + "gbdk-entries.zip","r") as zip_ref:
        zip_ref.extractall(unzipped_source+"gbdk-entries")
...
def build(prod: Production, entrypath: str, desired_extentions: list):
    ...
    if not os.path.exists(entrypath + prod.slug):
        ...
        if prod.url.endswith(".zip") or prod.url.endswith(".ZIP"):
            ...
            try:
                with zipfile.ZipFile(unzipped_source + "gbdk-entries/gb-manual-dir/" + prod.url,"r") as zip_ref:
                ...
                if utils.CLEANZIP: os.remove(unzipped_source + "gbdk-entries/gb-manual-dir/" + prod.url)
            ...
            logger.write("[INFO]", " Renaming from " + unzipped_source + "gbdk-entries/gb-manual-dir/" + prod.url + 
            " to " + "py_common/" + utils.BETA_FOLDER + "/" + prod.slug + "/" + prod.slug + "." + suffix + "...")
            os.rename(  unzipped_source + "gbdk-entries/gb-manual-dir/" + prod.url, 
...
def main():
    ...
    sources("MANUAL", unzipped_source + "gbdk-entries/gb-manual-source")

    # clean
    logger.write("[INFO]"," Removing unzipped_source folder..." + unzipped_source + "gbdk-entries")
    shutil.rmtree(unzipped_source + "gbdk-entries")

    logger.write("[INFO]", " GBDK importer has been executed.")
