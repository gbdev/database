...
try:
    with zipfile.ZipFile(unzipped_source + "gb-music-entries.zip","r") as zip_ref:
        zip_ref.extractall(unzipped_source+"gb-music-entries")
...
def build(prod: Production, entrypath: str, desired_extentions: list):
    ...
    if not os.path.exists(entrypath + prod.slug):
        ...
        if prod.url.endswith(".zip") or prod.url.endswith(".ZIP"):
            ...
            try:
                with zipfile.ZipFile(unzipped_source + "gb-music-entries/gb-manual-dir/" + prod.url,"r") as zip_ref:
                ...
                if utils.CLEANZIP: os.remove(unzipped_source + "gb-music-entries/gb-manual-dir/" + prod.url)
            ...
            logger.write("[INFO]", " Renaming from " + unzipped_source + "gb-music-entries/gb-manual-dir/" + prod.url + 
            " to " + "py_common/" + utils.BETA_FOLDER + "/" + prod.slug + "/" + prod.slug + "." + suffix + "...")
            os.rename(  unzipped_source + "gb-music-entries/gb-manual-dir/" + prod.url, 
...
def sources(type_of_source, path):
    ...
    for l in lines:
        ...
        if type_of_source.upper() == "AUTO":
            ...
            utils.build(prod, entrypath, ["GB", "GBC", "GBA"])            
        else:
            prod = Production(name, slug, dev, platform.upper(), typetag, screenshot, [None], url=data[4])
            build(prod, entrypath, ["GB", "GBC", "GBA"])
        
        logger.write("[INFO]", " Making JSON... ")
        utils.makeJSON(prod, entrypath)


def main():
    logger.write("[INFO]", " Building from manual sources...")
    sources("MANUAL", unzipped_source + "gb-music-entries/gb-manual-source")

    # clean
    logger.write("[INFO]"," Removing unzipped_source folder..." + unzipped_source + "gb-music-entries")
    shutil.rmtree(unzipped_source + "gb-music-entries")

    logger.write("[INFO]", " Music software script has been executed.")
